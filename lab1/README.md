...
**Aim**: to understand the basics of VHDL (VHSIC Hardware Description Language) and learn how to design, simulate, and               verify digital circuits using an open-source simulation tool.

 **Objective**:
To get familiar with VHDL syntax and structure
To write a basic VHDL design (example: buffer circuit)
To create a testbench for simulation
To simulate the design using an open-source simulator
To observe and analyze output waveforms
...
In this lab, we will primarily use the Dataflow style for its simplicity. Behavioral and Struc-
tural modeling will be explored in later labs.
...
2. std logic vector: Used to represent a group of bits (a bus). For example, std logic vector(7
downto 0) represents an 8-bit bus. The range 7 downto 0 means bit 7 is the most sig-
nificant bit (MSB).

3. Signals: Internal wires inside an architecture. They are declared between the architecture
... is line and the begin keyword.
...
**Output Observation**:
When input A = 0 → Output Y = 0
When input A = 1 → Output Y = 1
Output follows input directly (buffer behavior)

**Result**: The buffer circuit was successfully designed and simulated using VHDL. The waveform verified that the output               follows the input correctly.

**Conclusion**:This experiment helped in understanding:
            Basic VHDL programming structure
            Entity and architecture concepts
            Testbench creation
            Digital circuit simulation using open-source tools.
